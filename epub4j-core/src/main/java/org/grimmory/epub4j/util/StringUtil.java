 ...
 *
 * <p>Most of the functions herein are re-implementations of the ones in apache commons StringUtils.
 * The reason for re-implementing this is that the functions are fairly simple and using my own
...
public class StringUtil {
  ...
  /**
   * Changes a path containing '..', '.' and empty dirs into a path that doesn't. X/foo/../Y is
   * changed into 'X/Y', etc. Does not handle invalid paths like "../".
   *
   * @param path
  ...
  public static boolean endsWithIgnoreCase(String source, String suffix) {
    ...
    return source
        .substring(source.length() - suffix.length())
        .toLowerCase()
   ...
   *
   * <p>If the text does not contain the given separator then the given text is returned.
   *
   * @param text
   * @param separator
   ...
   *
   * <p>If the text does not contain the given separator then "" is returned.
   *
   * @param text
   * @param separator

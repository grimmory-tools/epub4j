...
public class HHCParser {
  ...
  public static List<TOCReference> parseHhc(InputStream hhcFile, Resources resources)
    ...
    Node ulNode =
        (Node) xpath.evaluate("body/ul", hhcDocument.getDocumentElement(), XPathConstants.NODE);
    return processUlNode(ulNode, resources);
  }
    ...
    for (int i = 0; i < children.getLength(); i++) {
      ...
      if ("li".equals(node.getNodeName())) {
        ...
        if (result.isEmpty()) {
          ...
          result.getLast().getChildren().addAll(childTOCReferences);
        }
      }
    }
    return result;
  ...
  /**
   * Processes a CHM object node into a TOCReference If the local name is empty then a TOCReference
   * node is made with a null href value.
   *
   * <p><object type="text/sitemap"> <param name="Name" value="My favorite section" /> <param
   * name="Local" value="section123.html" /> <param name="ImageNumber" value="2" /> </object>
   *
   * @param objectNode
   * @return A TOCReference of the object has a non-blank param child with name 'Name' and a
   *     non-blank param name 'Local'
   */

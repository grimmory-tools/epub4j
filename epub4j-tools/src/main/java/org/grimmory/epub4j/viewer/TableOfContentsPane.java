...
public class TableOfContentsPane extends JPanel implements NavigationEventListener {
  ...
  @Serial private static final long serialVersionUID = 2277717264176049700L;

  private final Map<String, Collection<DefaultMutableTreeNode>> href2treeNode = new HashMap<>();
  private final JScrollPane scrollPane;
  private final Navigator navigator;
  private JTree tree;

  /**
   * Creates a JTree that displays all the items in the table of contents from the book in
   * SectionWalker. Also sets up a selectionListener that updates the SectionWalker when an item in
   ...
   * @author paul
   */
  private record TOCItem(TOCReference tocReference) {
  ...
  private void addToHref2TreeNode(Resource resource, DefaultMutableTreeNode treeNode) {
    ...
    Collection<DefaultMutableTreeNode> treeNodes =
        href2treeNode.computeIfAbsent(resource.getHref(), k -> new ArrayList<>());
  ...
  public void navigationPerformed(NavigationEvent navigationEvent) {
    ...
    Collection<DefaultMutableTreeNode> treeNodes =
        href2treeNode.get(navigationEvent.getCurrentResource().getHref());
        ...
        new MouseAdapter() {
          ...
          public void mouseClicked(MouseEvent me) {
            ...
            navigator.gotoResource(
                tocItem.getTOCReference().getResource(),

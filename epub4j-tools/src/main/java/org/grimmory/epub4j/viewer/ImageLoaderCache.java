...
/**
 * This class is a trick to get the JEditorKit to load its images from the epub file instead of from
 * the given url.
 *
 * <p>This class is installed as the JEditorPane's image cache. Whenever it is requested an image it
 * will try to load that image from the epub.
 *
 * <p>Can be shared by multiple documents but can only be <em>used</em> by one document at the time
 * because of the currentFolder issue.
 *
 * @author paul
 */
class ImageLoaderCache extends Dictionary<String, Image> {
  ...
  private static final System.Logger log = System.getLogger(ImageLoaderCache.class.getName());
  ...
  public void initImageLoader(HTMLDocument document) {
    try {
      document.setBase(URI.create(ImageLoaderCache.IMAGE_URL_PREFIX).toURL());
  ...
  private String getResourceHref(String requestUrl) {
    ...
    resourceHref = BACKSLASH_PATTERN.matcher(resourceHref).replaceAll("/");
    return resourceHref;
  }
